...
def pathdraw():
    ...
    spotPosition_list, spotFull_list, spotBranch_list = getSpotPosition(transferStation)
    pathStartStation, inCarNumber = getStartStationPath(inCarNumber[0:4], startTrainHeadto, destiTrainHeadto, spotFull_list, spotBranch_list, getPathCount=1)
    response = drawPath(spotPosition_list, pathStartStation, transferStation)
    imgList.append(response)
    spotPosition_list, spotFull_list, spotBranch_list = getSpotPosition(destiStation)
    inCarNumberCount, endPointOrder = getDestiStationPath(spotFull_list, destiTrainHeadto, inCarNumber, endPoint)
    pathDestiStation = dijkstra(mat = spotBranch_list, begin = inCarNumberCount, end = endPointOrder)
    response = drawPath(spotPosition_list, pathDestiStation[:-1], destiStation)
    imgList.append(response)
    return imgList[2]
    # inCarNumberCount, endPointOrder = getDestiStationPath(spotFull_list, destiTrainHeadto, inCarNumber, endPoint)
    # pathDestiStation = dijkstra(mat = spotBranch_list, begin = inCarNumberCount, end = endPointOrder)
    # response = drawPath(spotPosition_list, pathDestiStation[:-1], destiStation)
    # response = drawPath(spotPosition_list, pathStartStation, destiStation)
    # imgList.append(response)
